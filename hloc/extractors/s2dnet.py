...
WEIGHTS_PATH = Path(__file__).parent / "weights" / "s2dnet_weights.pth"  # weights path
...
class S2DNet(BaseModel):
    ...
    def _init(self, conf):
        ...
        vgg16 = models.vgg16(weights=conf["pretrained"] == "imagenet")
        ...
        if conf["pretrained"] == "s2dnet":
            ...
            state_dict = {
                k: v for k, v in state_dict.items() if v.shape == params[k].shape
            }
